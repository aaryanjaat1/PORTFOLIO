...
import { X, Play, ArrowRight, Maximize2, Activity, Clock, Layers, Calendar } from 'lucide-react';
  ...
  return (
    <div className="pt-32 md:pt-40 px-4 md:px-6 min-h-screen relative overflow-hidden bg-black">
      ...
      <div className="container mx-auto">
        ...
        {/* Masonry-style grid with strict aspect ratio adherence */}
      ...
      {/* Cinematic Responsive Modal with Aspect Ratio Correction */}
      <AnimatePresence>
        {selectedProject && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div 
              ...
              className={`w-full bg-neutral-950 rounded-[2rem] md:rounded-[4rem] border border-white/5 overflow-hidden relative shadow-2xl flex flex-col lg:flex-row
                ${selectedProject.aspectRatio === '9:16' ? 'max-w-4xl max-h-[90vh]' : 'max-w-7xl'}
              `}
            >
              <button 
                onClick={() => setSelectedProject(null)}
                className="absolute top-6 right-6 z-[110] w-10 h-10 md:w-14 md:h-14 glass rounded-full flex items-center justify-center text-white hover:bg-white hover:text-black transition-all duration-500"
              >
              ...
              `}>
                ...
                <div className="space-y-8 md:space-y-12 overflow-y-auto custom-scrollbar pr-2">
                  {selectedProject.metrics && (
                    <div className="p-6 md:p-8 rounded-2xl md:rounded-[2.5rem] flex items-center gap-4 md:gap-6 border-2 border-white/5 bg-white/5">
